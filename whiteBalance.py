...
def grayWorldAlgo(image):
    avgA = np.average(image[:,:,1])
    avgB = np.average(image[:,:,1])
    image[:,:,1] = image[:,:,1] - ((avgA-128) * (image[:,:,0]/255.0) * 1.2)
    image[:,:,2] = image[:,:,2] - ((avgB-128) * (image[:,:,0]/255.0) * 1.2)
    GWWhiteBalancedImage = cv2.cvtColor(image, cv2.COLOR_LAB2BGR)
